...
-- Trigger d'incrémentation de l'identifiant relatif NUM_OCCURRENCE 
CREATE OR REPLACE TRIGGER TRE_NUMERO_OCCURENCE 
BEFORE INSERT ON OCCURRENCE
FOR EACH ROW 
DECLARE 
    num_occ NUMBER;
BEGIN 
  IF(:NEW.NUM_OCCURRENCE IS NULL OR :NEW.NUM_OCCURRENCE = 0) THEN 
        SELECT COALESCE(MAX(NUM_OCCURRENCE), 0) + 1 INTO num_occ
        FROM OCCURRENCE 
        WHERE NUM_EVENEMENT = :NEW.NUM_EVENEMENT;
        :NEW.NUM_OCCURRENCE := num_occ; 
  END IF; 
END;